...
The RSVP form is protected with [Cloudflare Turnstile](https://developers.cloudflare.com/turnstile/) — an invisible, privacy-friendly alternative to reCAPTCHA.

### Setup

1. Create a widget at [Cloudflare Turnstile Dashboard](https://dash.cloudflare.com/?to=/:account/turnstile)
2. Add your domain to the allowed hostnames
3. Set the widget mode to **Non-Interactive** (recommended to avoid Private Access Token issues)
4. Add the keys to your environment variables:
```env
TURNSTILE_SECRETKEY=your_secret_key_here
```

5. Add the public Site Key to `SectionForm.astro`:
```astro
...
- The Turnstile script is loaded in `Layout.astro` via CDN
- On form submit, the token generated by Turnstile is sent alongside the form data
- The `/api/confirmar` endpoint verifies the token with Cloudflare before processing the RSVP
- If verification fails, the request is rejected with a 400 response
